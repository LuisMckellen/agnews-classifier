...
t = df["title"].astype(str).str.strip() + " " + df["description"].astype(str).str.strip()

loose  = t.str.contains(r"#\d{2,4};|(?:lt|gt|quot|amp|nbsp|apos);", regex=True)
strict = t.str.contains(r"#\d{2,4};|\b(?:lt|gt|quot|amp|nbsp|apos);", regex=True)

extra = loose & ~strict
print(extra.sum())
for i in df.index[extra]:
    print(i, df.loc[i, "label"], repr(t[i]))